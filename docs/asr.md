...
Dispatched in `internal/transcriber/engine.go:Job.runASR`; never branch inside an engine function. Values: `whisper-onnx` (default; `whisper` accepted as legacy alias), `zipformer`, `moonshine`, `parakeet`, `sensevoice`, `canary`, `nemo-ctc`. Set via `shared.Config.Engine` / env `WT_ENGINE` / `JobSpec.Engine`.
