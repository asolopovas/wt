...
Default: **don't add anything**. Most footguns are better captured in code (named helper, error message, test) or commit messages.
...
- Pick the right `docs/` file by topic; create a new one only if no existing file fits.
- One line per rule. Imperative voice (Always / Never / Ask). No narrative, no story.
- Compress aggressively: drop reasoning unless the _why_ prevents the next person from "fixing" it back. Drop dates and benchmark numbers unless they gate a decision.
- Only touch `AGENTS.md` itself if the rule is cross-cutting (touches >2 topics) or changes the command surface.
- Never commit doc edits unless instructed.
