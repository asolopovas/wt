...
Never use `grep ... | sed -E "s/.../\1/"` inside a Task `sh:` var — mvdan/sh mangles the `\1` backref. Use `awk -F'[\047\042]' '/pattern/{print $2}'` instead (`\047` and `\042` are octal for `'` and `"`, making the pattern quote-agnostic). Same shell also panics on fd>=10 redirects (`exec 9>>file`) — use a cp-retry loop instead of fd-based lock probing on Windows.

Never use `awk -F"\x27"` (single-quote-only) for VERSION extraction in Taskfile.yml. Editor auto-formatters (biome, prettier-yaml, etc.) silently convert `VERSION: '0.0.7'` to `VERSION: "0.0.7"` and the awk extraction returns empty — every `_install-host`, `_build-host`, `_release-stable`, `installer-exe`, `linux-deb`, etc. that re-reads VERSION at task time (because of `task release` post-bump re-read) then sees an empty string and produces `dist/wt__amd64.deb`-style filenames or copies the wrong artifacts. Always use the quote-agnostic form `awk -F'[\047\042]' '/^  VERSION:/{print $2; exit}' Taskfile.yml`. The canonical on-disk form is single quotes (`VERSION: '0.0.7'`) because the `bump` task's PowerShell `-replace` substitution is keyed on single quotes; if you must edit the line, restore single quotes after.
...
- **Mirror initialisation order**: panel-level state mutations (e.g. `refreshLanguageOptions` filtering the master select's `Options` based on active engine) run during `settingsPanel.build()` via `defer`. Mirror factories like `newLangSelectMirror`/`newModelSelectMirror` are called _later_ by `app.go`/`app_android.go` when the transcode tab is constructed. Always seed a new mirror from the master widget's already-filtered `Options` (and copy `Disabled()` state) — not the raw global slice. Otherwise the mirror surfaces stale unfiltered options at tap time (LimitSelect.Tapped reads Inner.Options on each tap).
...
On Android, sherpa-onnx CLIs are bundled as `lib*.so` (e.g. `libsherpa-diar.so`, `libsherpa-asr.so`) so the Android packager installs them under `/data/app/<pkg>/lib/arm64/`. Binary discovery must check that path _before_ `exec.LookPath` — see `findSherpaBinary` / `findZipformerBinary`.
