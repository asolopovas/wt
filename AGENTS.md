...
**NeMo streaming Sortformer v2** (`nvidia/diar_streaming_sortformer_4spk-v2`, GPU, ≤4 speakers, auto-detects count). Affinity restriction (`sched_setaffinity` to a core subset) is tempting for splitting UI vs compute but on heterogeneous big.LITTLE SoCs (e.g. Snapdragon 8 Gen 3: cores 0-3 @ 1.96 GHz silver, 4-6 @ 2.59 GHz gold, 7-8 @ 2.9 GHz, 9 @ 3.2 GHz prime), reserving the top N cores for UI confines whisper to the slower silver cluster and can cause >10x throughput drop. Stick to nice differential + `ReserveTopCores(2)` for the locked Go thread; do NOT propagate the reserved mask to whisper worker threads.
...
- Put gh-CLI / multi-step git-tag-then-publish flows in a `|` block under Task's embedded mvdan/sh on Windows. The interpreter silently aborts after `git push origin <tag>` (zero output, exit 1, no `set -e` involved) before reaching subsequent commands. Extract such flows into a standalone bash script invoked via `C:/msys64/usr/bin/bash.exe scripts/<name>.sh` and run the gh calls there.
- Spawn child shells from msys64 bash (`bash foo.sh`) when the parent script needs to share env vars with the child. msys2 strips most user-set env (including exported `GH_CONFIG_DIR`, `FOO=…`) on subprocess fork; only a short whitelist (HOME, PATH, MSYSTEM, SYSTEMDRIVE, SYSTEMROOT, TERM, WINDIR) propagates. Inline the logic instead, or pass values via positional args.
- Rely on Windows env vars (`APPDATA`, `USERPROFILE`, `LOCALAPPDATA`) inside msys64 bash invoked from Task — they are not inherited. Reconstruct paths via `cygpath -H` + `whoami` (e.g. `GH_CONFIG_DIR="$(cygpath -H)/$(whoami)/AppData/Roaming/GitHub CLI"` so `gh` finds its auth config when run from msys64 bash).
- Have `task install` (called from `bump`/`release`) build the Windows installer. The build task gates `installer-exe` on `[ -z "{{.ONLY}}" ]`, and install passes `ONLY=both`, so the installer is intentionally skipped during install. `task release` must invoke `task installer-exe` explicitly after `task build` — otherwise `dist/wt-setup-<ver>.exe` is missing and the upload step fails with "no artifacts found".

## Fyne UI update rate on Android

(also: long-running compute drives `Process.setThreadPriority`/`/dev/cpuctl` quirks; see "Background CPU work on Android" below.)

On Android, every `fyne.Do(...)` callback that touches a widget (`SetValue`, `Text=...`+`Refresh`) ends up driving a GLES repaint through SurfaceFlinger. The render thread — not CPU — is the scarce resource. ~15+ `fyne.Do`/sec from background goroutines (smoothers, status pollers, progress tickers) saturates the frame pipeline and the user sees timer/animation lag while total process CPU is ~2-10%. Two rules: (1) cap background-driven update rates at 4 Hz / 250 ms; 100 ms tickers are wasteful on mobile. (2) skip the `fyne.Do` call entirely when nothing actually changed since last apply (compare `current` to `lastApplied`, status string to `lastStatus`) — unconditional fires multiply waste by N. Foreground-service `NotificationManager` updates via binder follow the same logic: 2 Hz is plenty for a status bar progress notification.

## Background CPU work on Android

Long CGo compute (whisper.cpp transcribe, sherpa diarize) freezes the Fyne UI on phones even when run in a goroutine. CPU affinity (`sysstats.ReserveTopCores`) alone is insufficient — the Linux scheduler still time-slices UI vs compute on shared cores under thermal/cache pressure. Industry pattern (ExoPlayer / TFLite / MediaPipe): drop the compute thread to `nice=10` (`THREAD_PRIORITY_BACKGROUND`) so CFS only schedules it when foreground has slack. `sysstats.SetCurrentThreadBackground()` does this via `setpriority(PRIO_PROCESS, tid, 10)` plus optional write to `/dev/cpuset/background/tasks`. Linux nice IS inherited by `pthread_create` (clone+CLONE_THREAD), so dropping the locked Go OS thread's nice BEFORE entering CGo propagates to whisper/sherpa worker threads spawned inside. **But** that only covers threads spawned AFTER the drop — whisper's persistent thread pool created during `whisper.New()` (model load) is born before the per-job pinner starts and stays at NI=-10 unless explicitly re-niced. Android's foreground-service promotion also periodically re-boosts threads back to NI=-10. So the per-job thread pinner must (a) start before/at-the-same-time as model load, AND (b) run continuously, AND (c) iterate ALL non-self TIDs every tick filtering by `/proc/self/task/<tid>/comm` against a UI/system whitelist (RenderThread, GLThread, ANGLE-*, binder:*, HwBinder*, Jit*, Finalizer*, HeapTaskDaemon, NDK MediaCodec, app process name) — NOT just diff against a TID baseline snapshot. Snapshot-diff strategies will leak whisper's pre-existing pool. Verify on device with `adb shell top -H -p <pid>` — if any heavy `Thread-N` worker shows `NI -10` instead of `NI 5`, the pinner is incomplete. Always pair with `runtime.LockOSThread` so the priority change applies to a stable kernel TID. Default Android whisper threads kept low (≤4) — more threads = more contention regardless of nice.
