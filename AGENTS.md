...
Android screenshot capture (for visual GUI verification): always quote the remote command and disable msys path conversion on the pull. The reliable one-liner is `adb shell "screencap -p /sdcard/s.png" && MSYS_NO_PATHCONV=1 adb pull /sdcard/s.png _tmp/s.png` then read `_tmp/s.png` with the Read tool. Without quotes, `adb shell screencap -p ...` makes adb consume `-p` as its own flag and `screencap` prints its usage instead of capturing. Without `MSYS_NO_PATHCONV=1`, msys rewrites `/sdcard/...` to `C:/Program Files/Git/sdcard/...` on the pull and fails with `failed to stat remote object`. Save to `_tmp/` (gitignored) per the scratch-artifacts rule — not the repo root, not `C:\tmp` (Read tool can't reach it).
