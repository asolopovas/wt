...
func (p *Params) SetSuppressNST(v bool) {
	p.suppress_nst = toBool(v)
}

func (p *Params) SetNoSpeechThold(t float32) {
	p.no_speech_thold = C.float(t)
}

func (p *Params) SetLogprobThold(t float32) {
	p.logprob_thold = C.float(t)
}

func (p *Params) SetSuppressRegex(pattern string) {
	p.suppress_regex = C.CString(pattern)
}

func (p *Params) SetStrategy(s C.enum_whisper_sampling_strategy) {
	p.strategy = s
}
